...
//  copyBlock
//  function copies a string of size n into a binary integer array
//    input: [ input str pointer, output int pointer, size of string]
//    output: none
static void copyBlock(char *src, int *dest, int size);

//  copyBlock_HEX
//  function copies a HEX string of size n into a binary integer array
//    input: [ input str pointer, output int pointer, size of HEX string]
//    output: true if input is in HEX, false if input was in another alphabet
static bool copyBlock_HEX(char *src, int *dest, int size);

//  printBlock
//  function prints an int array of size n
//    input: [ input int pointer, size of array]
//    output: none
static void printBlock(int *block, int size);

//  XOR
//  function XORs two integer arrays of size in
//    input: [ input str pointer_1, input str pointer_2, output int pointer, size of arrays]
//    output: none
static void XOR(int *input_1, int *input_2, int *output, int size);

//  leftRotate
//  function left rotates an integer string
//    input: [ int array pointer, size of array]
//    output: none
static void leftRotate(int * sequence, int size);

//  rightRotate
//  function right rotates an integer string
//    input: [ int array pointer, size of array]
//    output: none
...
//    input: [ x byte of key, key, output byte]
//    output: none
static void K(int x, int *key_sequence, int *output);

//  G
//  function generates G values for F function
//    input: [ input block, key 1, key 2, key 3, key 4, key 5, key 6, output block]
//    output: none
static void G(int *w, int *k1, int *k2, int *k3, int *k4, int *output);

//  F
//  round function for twofish
//    input: [ input block_1, input block_2, key, round number output block_1, output block_2]
//    output: none
static void F(int *R_0, int *R_1, int *key_sequence, int round_number, int *output_F_0, int *output_F_1);

//  split_R
//  Splits 64-bit block to four 16-bit blocks
//    input: [ output 16-bit block_1, output 16-bit block_2, output 16-bit block_3, output 16-bit block_4, input 64-bit block]
//    output: none
static void split_R(int *R_0, int *R_1, int *R_2, int *R_3, int *R);

//  f_table
//  Finds f-table values based on byte
//    input: [ input byte, output byte ]
//    output: none
...
static int add_binary(int *b1, int *b2, int *b3);
  ...
  int encode(   int (*readFunc)(void* context),
    ...
    // while not at end of the file, encrypt
    while ((read_result = readInBits(input, 64)) == 0) {

      // convert plaintext to binary and save into input_sequence[64] as an int array
      copyBlock(input->block, input_sequence, 8);	
  ...
  void K(int x, int *key_sequence, int *output) {
    leftRotate(key_sequence, 64);
    for (int i = 0; i < 8; i++) 
      output[i] = key_sequence[((56 - ((x % 8) * 8)) + i)];
    return;
  }
  ...
  void F(int *R_0, int *R_1, int *key_sequence, int round_number, int *output_F_0, int *output_F_1) {
    ...
    K(4 * round_number, key_sequence, gk_1);
    K((4 * round_number) + 1, key_sequence, gk_2);
    K((4 * round_number) + 2, key_sequence, gk_3);
    K((4 * round_number) + 3, key_sequence, gk_4);

    // computing keys for second call to G()
    K(4 * round_number, key_sequence, gk_5);
    K((4 * round_number) + 1, key_sequence, gk_6);
    K((4 * round_number) + 2, key_sequence, gk_7);
    K((4 * round_number) + 3, key_sequence, gk_8);

    // computing keys for F_0 and F_1
    K(4 * round_number, key_sequence, fk_1);
    K((4 * round_number) + 1, key_sequence, fk_2);
    K((4 * round_number) + 2, key_sequence, fk_3);
    K((4 * round_number) + 3, key_sequence, fk_4);
